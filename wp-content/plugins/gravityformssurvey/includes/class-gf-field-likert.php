	...
	public function get_likert_rows( $value, $disabled_text, $form_id ) {
		...
		for ( $i = 1; $i <= $num_rows; $i ++ ) {
			...
			foreach ( $this->choices as $choice ) {
				...
				$content .= sprintf( "<td data-label='%s' class='%s'><input name='%s' type='radio' value='%s' %s id='%s' %s %s %s/></td>", esc_attr( $choice['text'] ), $cell_class, $input_name, esc_attr( $field_value ), $checked, $input_id, $disabled_text, $this->get_tabindex(), $this->get_conditional_logic_event( 'click' ) );
				$choice_id ++;
			}
