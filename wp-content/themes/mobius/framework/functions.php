...
function display_slider() {
    ...
    if( is_front_page() && $slider = get_setting( 'main_slider', false ) )
        echo do_shortcode( '[slider id=' . $slider . ']' );
}

function spyropress_wp_nav_menu_objects( $items, $args ) {
    
    if( 'primary' != $args->theme_location || get_setting( 'disable_home' ) ) return $items;
    
    $url = is_front_page() ? '#fwslider' : home_url();
    $class = !is_front_page() ? ' class="external"' : '';
    $li_class = is_front_page() ? ' class="current"' : '';
    return '<li' . $li_class . '><a href="' . $url . '"' . $class . '>' . get_setting( 'home_label' ) . '</a></li>'.$items;
}
?>