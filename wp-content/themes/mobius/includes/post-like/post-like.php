...
function like_scripts() {
	wp_enqueue_script( 'like-post', get_template_directory_uri().'/includes/post-like/post-like.js', array('jquery'), '1.0', 1 );
...
function like_scripts_register_styles() {	 
	wp_register_style('like-styles',  get_template_directory_uri() . '/includes/post-like/post-like.css');
	wp_enqueue_style('like-styles');
}
add_action('wp_print_styles', 'like_scripts_register_styles');
...
function getPostLikeLink( $post_id ) {
	...
	if ( AlreadyLiked( $post_id )) {
		$class = ' liked';
		$title = __( 'Unlike', 'mobius'  );
		$heart = '<i class="fa fa-heart"><i class="icon-to-x"></i></i>';
	} else if ($count == 0) {
		$class = ' no-liked';
		$title = __( 'Like', 'mobius'  );
...
function show_user_likes( $user ) { ?>        
    <table class="form-table">
        <tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'mobius' ); ?></label></th>
			<td>
            <?php
			...
			if ( $count > 0 ) {
				...
				echo "<p>" . _e( 'You don\'t like anything yet.', 'mobius' ) . "</p>\n";
			} ?>
            </td>
		</tr>
    </table>
